...
class FeiKeyHintTest {
    ...
    fun `密键按图2最宽行取列数`() {
        ...
        assertEquals("爪爫丬爿", FeiKeyHint.forKey("z")!!.rows[2])
        assertEquals("長镸罒巛", FeiKeyHint.forKey("z")!!.rows[3])
    ...
    fun `字根格子按宽高较短边取字号 末行不撑满列`() {
        ...
        assertEquals(4, z.colCount)
        assertEquals(5, z.rowCount)
        assertEquals(7.5f, z.cellWidthDp, 0.01f)
